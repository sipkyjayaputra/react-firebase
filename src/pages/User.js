...
const User = ({authenticatedUser}) => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const userData = usersCollection.docs
        .filter((doc) => doc.data().role === 3) // Filter documents based on role === 3
        .map((doc) => ({
          id: doc.id,
          ...doc.data(),
        }));
  ...
  const handleEditUser = async () => {
    try {
      if (selectedUser) {
        ...
        if (newUser.image) {
          // Generate a unique filename using current timestamp and original filename
          const uniqueFilename = `${getUnixTime(new Date())}_${
            newUser.image.name
          }`;

          // Delete the old image from Firebase Storage
          if(selectedUser.imageName !== undefined) {
